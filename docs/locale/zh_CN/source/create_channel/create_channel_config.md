...
由于文件的不同部分共同创建用于管理通道的策略，因此我们将在[其各自的教程](channel_policies.html)中讨论通道策略。
...
测试网络使用的`configtx.yaml`文件包含三个组织。可以添加到应用程序通道的两个组织是Peer组织Org1和Org2。OrdererOrg是一个Order组织，是Order服务的管理员。因为最佳做法是使用不同的证书颁发机构来部署Peer节点和Order节点，所以即使组织实际上是由同一公司运营，也通常将其称为Peer组织或Order组织。
...
如果检查`configtx.yaml`文件，您将看到三个功能组：
...
每个通道配置都在通道[共识者结合](../glossary.html#consenter-set)中包括Order节点。共识者集合是一组排序节点，它们能够创建新的块并将其分配给加入该通道的Peer节点。在通道配置中存储作为共识者集合的成员的每个Order节点的端点信息。

测试网络使用`configtx.yaml`文件的**Orderer**部分来创建单节点Raft Order服务。
...
Raft Order服务由可以参与共识过程的共识者列表定义。因为测试网络仅使用一个Order节点，所以共识者列表仅包含一个端点：
  ```yaml
  EtcdRaft:
      Consenters:
  ...
  共识者列表中的每个Order节点均由其端点地址以及其客户端和服务器TLS证书标识。如果要部署多节点Order服务，则需要提供主机名，端口和每个节点使用的TLS证书的路径。您还需要将每个排序节点的端点地址添加到`Addresses`列表中。
...
因为测试网络用于开发和测试，所以它使用由单个Order节点组成的Order服务。生产中部署的网络应使用多节点Order服务以确保安全性和可用性。要了解更多信息，请参阅[配置和操作Raft Order服务](../raft_configuration.html)。

## Channel

通道部分定义了用于管理最高层级通道配置的策略。对于应用程序通道，这些策略控制哈希算法，用于创建新块的数据哈希结构以及通道功能级别。在系统通道中，这些策略还控制Peer组织的联盟的创建或删除。

测试网络使用Fabric提供的默认策略，该策略要求大多数Order服务管理员需要批准对系统通道中这些值的更新。在应用程序通道中，更改需要获得大多数Order组织和大多数通道成员的批准。大多数用户不需要更改这些值。

## Profiles

`configtxgen`工具读取**Profiles**部分中的通道配置文件以构建通道配置。每个配置文件都使用YAML语法从文件的其他部分收集数据。 `configtxgen`工具使用此配置为应用程序通道创建通道创建交易，或为系统通道写入通道创世块。要了解有关YAML语法的更多信息，[Wikipedia](https://en.wikipedia.org/wiki/YAML)提供了一个入门的好地方。
...
系统通道定义了Order服务的节点以及Order服务管理员的组织集合。系统通道还包括一组属于区块链[联盟](../glossary.html#consortium)的Peer组织。联盟中每个成员的通道MSP都包含在系统通道中，从而允许他们创建新的应用程序通道并将联盟成员添加到新通道中。

该配置文件创建一个名为`SampleConsortium`的联盟，该联盟在`configtx.yaml`文件中包含两个Peer组织Org1和Org2。配置文件的`Orderer`部分使用文件的**Orderer: **部分中定义的单节点Raft Order服务。**Organizations: **部分中的OrdererOrg成为Order服务的唯一管理员。因为我们唯一的Order节点正在运行Fabric 2.x，所以我们可以将Order系统通道功能设置为`V2_0`。系统通道使用**Channel**部分中的默认策略，并启用`V2_0`作为通道功能级别。
...
Order服务将系统通道用作创建应用程序通道的模板。在系统通道中定义的Order服务的节点成为新通道的默认共识者集合，而Order服务的管理员则成为该通道的Order管理员。通道成员的通道MSP从系统通道转移到新通道。创建通道后，可以通过更新通道配置来添加或删除Order节点。您还可以更新通道配置以[将其他组织添加为通道成员](../channel_update_tutorial.html)。

`TwoOrgsChannel`提供了测试网络系统通道托管的联盟名称`SampleConsortium`。因此，`TwoOrgsOrdererGenesis`配置文件中定义的Order服务成为通道共识者集合。在`Application`部分中，来自联盟的两个组织Org1和Org2均作为通道成员包括在内。通道使用`V2_0`作为应用程序功能，并使用**Application**部分中的默认策略来控制Peer组织如何与通道进行交互。应用程序通道还使用**Channel**部分中的默认策略，并启用`V2_0`作为通道功能级别。
