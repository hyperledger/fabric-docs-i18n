.. note:: <div dir="rtl"> لطفاً مطمئن شوید که به مستنداتی متناسب با نسخه نرم افزار مورد استفاده خود نگاه می کنید. در سمت چپ صفحه می توانید لیبل نسخه را درقسمت بالای پنل پیمایش مشاهده کنید. همچنین می توانید آن را با استفاده از گزینه های انتخاب در پایین پنل پیمایش تغییر دهید.

</div>

پلتفرم مبتنی بر بلاکچین برای کاربردهای سازمانی و تجاری
========================================

.. image:: images/hyperledger_fabric_logo_color.png

پلتفرم دفترکل توزیع شده مجوزدار، که خاصیت  ماژولار بودن و انعطاف پذیری را برای مجموعه گسترده ای از کاربردهای صنعتی ارائه می دهد.



.. toctree::
   :maxdepth: 1

   whatis
   whatsnew
   key_concepts
   getting_started
   getting_started_run_fabric
   developapps/developing_applications
   tutorials
   deployment_guide_overview
   ops_guide
   upgrade
   command_ref
   architecture
   Fabric-FAQ
   CONTRIBUTING
   glossary
   releases
   questions
   status

<div dir="rtl">

.. note:: درصورت داشتن سوالی که در این مستندات به آن اشاره نشده و یا برخورد با مشکلاتی که در دستورالعمل ها ذکر نشده، لطفا از صفحه :doc:`questions` برای مشاهده راهنمایی ها و این که کجا می توانید کمک بیشتری پیدا کنید، دیدن فرمایید.

</div>

.. تحت مجوز Creative Commons Attribution 4.0 International
   https://creativecommons.org/licenses/by/4.0/
