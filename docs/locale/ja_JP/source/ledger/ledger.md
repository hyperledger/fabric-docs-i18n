...
もし、[basic asset transfer サンプルアプリケーション](../write_first_app.html)を実行したことがあるならば、この台帳を作ったことがあることになります。

Basicのサンプルアプリケーションは、それぞれ異なる識別子、色、サイズ、所有者、評価額をもつ6つの資産を作成します。
これが、最初の4つの資産が作られた後に台帳がどうなっているかです。

![ledger.transaction](./ledger.diagram.6.png) *台帳Lは、ワールドステートWとブロックチェーンBから構成されています。
Wは、ASSET1、ASSET2、ASSET3、ASSET4というキーの4つのステートを含んでいます。
Bは、0と1という2つのブロックを含んでおり、ブロック1は、T1、T2、T3、T4という4つのトランザクションを含んでいます。*

ワールドステートには、ASSET1、ASSET2、ASSET3、ASSET4に対応するステートが存在していることがわかります。
ASSET1は、青く(blue)サイズが5で、現在トモコ(Tomoko)によって所有されていることを表す値を持っており、
...
しかし、ブロック1は、T1、T2、T3、T4というトランザクションを含んでおり、これらがワールドステートのASSET1からASSET4までの初期ステートを作成したトランザクションに対応するものです。
