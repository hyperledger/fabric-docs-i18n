...
注意: このトピックでは、システムチャネルを持たないネットワーク上のチャネルを更新する手順について説明します。システムチャネルに関する情報を含むこのトピックのバージョンについては、[チャネル設定の更新](https://hyperledger-fabric.readthedocs.io/en/release-2.2/config_update.html)を参照してください。
...
[サンプルの`configtx.yaml`ファイルはこちら](http://github.com/hyperledger/fabric/blob/{BRANCH}/sampleconfig/configtx.yaml)をご覧ください。
...
チャネル設定更新は、更新されるチャネルの設定にのみ適用されるため、チャネル名 `CH_NAME` は、更新されるすべてのチャネルに設定する必要があります。
...
`jq` の使用方法の例については、[チャネルケーパビリティの更新](updating_capabilities.html#Create-a-capabilities-config-file) を確認してください。
