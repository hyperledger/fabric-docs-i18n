...
<<<<<<< HEAD
別の例として、もしあなたが ``startKey='CAR0',endKey='CAR999'`` を使用するなら、 ``getStateByRange`` は ``CAR0``　（包括的） と ``CAR999`` 　（排他的）の間のキーを持つ車を辞書順で検索します。
...
>>>>>>> release-2.2_CreateChannelOverview
...
<<<<<<< HEAD
これで、いくつかのクエリーを実行し、少しコードを追加したので、台帳を更新する準備ができました。最初に **新しい** 自動車を作成しましょう。
=======
