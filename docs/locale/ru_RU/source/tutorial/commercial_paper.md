...
суть коллаборации двух разных организаций, хоть и независимых, но действующих
...
Прежде, чем начать, вам нужно установить требуемое программное обеспечение. Мы постарались свести
требования к минимуму.

Вам **необходимо** установить:

  * [**Node**](https://github.com/hyperledger/fabric-sdk-node#build-and-test)
    Файл README для Node.js SDK содержит актуальный список требований.

 **Считается полезным**, и вам будет легче, если вы также установите:

  * Текстовый редактор кода, например:
    [**Visual Studio Code**](https://code.visualstudio.com/) версии 1.28 или выше. 
...
* Показать вывод однорангового узла, службы упорядочения и УЦ из вашей сети.
* Одобрить чейнкод от имени администратора MagnetoCorp и от имени администратора DigiBank.
* Запустить приложения от имени Изабеллы и Баладжи, использующих смарт-контракт для
...
Тестовая сеть состоит из двух равноправных организаций и одной упорядочивающей организации.
Две равноправных организации управляют по одному одноранговому узлу каждая, в то
время как упорядочивающая организация управляет единственным узлом службы упорядочения на
протоколе Raft. Также мы создадим в тестовой сети единственный канал с именем
...
и одной упорядочивающей организации. Каждый компонент запущен как контейнер Docker.*
...
Наличие двух организаций в тестовой сети позволяет нам взаимодействовать с реестром
блокчейна от имени этих организаций, у каждой из которых есть свой однограновый узел.
В данном руководстве Org1 действует как DigiBank и Org2 как MagnetoCorp.
...
Повторимся: вы загрузили репозиторий образцов Hyperledger Fabric из GitHub
...
Эта команда выдаст тот же пакетный идентификатор, что и команда установки. Вы
...
Изабелла может использовать его имя и секретную фразу для генерации криптографического материала
...
производственных задач, оператор сети предоставит имя и секретную фразу клиентского идентификатора,
...
и закрытого, а затем отправляет **Запрос на подпись сертификата** в УЦ.
Если имя идентификатора и секретная фраза, которые отправляет Изабелла в УЦ, совпадут с хранящимися там, то
...
Программа `addToWallet.js` внесла идентифицирующую информацию для `balaji` в его
...
Баладжи теперь может запустить `buy.js`, чтобы послать транзакцию, которая передаст
...
очистки среды можете использовать скрипт. В командном окне перейдите в директорию с
...
Чтобы понять подробности устройства и работы приложений и смарт-контрактов из данного
