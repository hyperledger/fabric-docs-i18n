...
* **Тип БД.**
  Некоторые каналы в сети могут требовать данные в формате, пригодном для чтения в виде :doc:`couchdb_as_state_database`, в том время как для иных из соображений скорости исполнения, будет достаточно, чтобы все равноправные узлы использовали  LevelDB. Отметим, что из-за некоторых отличий CouchDB и LevelDB, всеравноправные узлы сети одновременно могут только один выбранный тип базы данных.

* **Каналы и приватная информация.**
  Для некоторых сетей использование :doc:`каналов` может оказаться наилучшим вариантом обеспечения конфиденциальности и изолированности для некоторых конкретных транзакций. В других же сетях для целей приватности может больше подойти единый кана с использованием :doc:`private-data/private-data`.
