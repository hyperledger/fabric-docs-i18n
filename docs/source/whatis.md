...
Hyperledger Fabric 是一个许可平台，通过其通道架构和 [私有数据](./private-data/private-data.html) 特性实现保密。在通道中，Fabric 网络的参与者会建立一个子网络，在子网络中他们的交易是互相可见的。而且，只有通道中的成员的节点才有智能合约（链码）和数据交易权限，从而保证了双方的隐私和信任。私有数据可以在通道中的成员间使用集合（collections），实现了和通道一样的安全性，但是省去了创建和维护通道的麻烦。
...
Fabric 目前提供了一种基于 [Raft 协议](https://raft.github.io/raft.pdf) [`etcd` 库](https://coreos.com/etcd/)的 CFT 排序服务。更多当前可用的排序服务请查阅 [排序服务概念文档](./orderer/ordering_service.html)。

另外，请注意，这些并不相互排斥。一个 Fabric 网络中可以有多种排序服务以支持不同的应用或应用需求。

## 性能和可扩展性

一个区块链平台的性能可能会受到许多因素的影响，例如交易大小、区块大小、网络大小以及硬件限制等。Hyperledger Fabric [性能和规模工作组（Performance and Scale working group）](https://wiki.hyperledger.org/display/PSWG/Performance+and+Scale+Working+Group) 目前正在开发一个叫 [Hyperledger Caliper](https://wiki.hyperledger.org/projects/caliper) 的基准测试框架。

已经发表了很多研究和测试 Hyperledger Fabric 性能的论文。最新的一篇是 [将 Fabric 的每秒交易量扩容到两万（Scaled Fabric to 20,000 transactions per second）](https://arxiv.org/abs/1901.00910)。
