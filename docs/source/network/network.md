...
在这个话题中，我们看到了不同的组织间是如何共享他们的基础设施来提供一个集成的 Hyperledger Fabric 区块链网络。我们看到了这些基础设施是如何被组织到通道中来提供可以独立管理的私有通信机制。我们也看到了像客户端应用程序、管理员、Peer 节点和排序节点这些操作者，是如何根据不同组织的 CA 颁发的证书被识别出来。我们也看到了在网络和通道资源上，定义组织操作者们一致同意的权限策略的重要性。
